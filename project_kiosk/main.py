...
drink_name = {1: "스무디", 2: "버블티", 3: "에이드", 4: "생과일쥬스", 5: "탄산"}
...
while True:
    ...
    if choice == 1:    # Coffee
        ...
        for i in range(len(coffee_name)):
            print(f"◆◇ {i+1}.{coffee_name[i+1]} ({coffee_price[i+1]})")
    ...
    if flag == 1:
        ...
        for price in price_save:
            total_price += price
        print(f"MSG: 주문하신 메뉴는 {len(menu_save)}개로 총 결제 금액은 {total_price}원 입니다.")
